...
// Sistema de gamificação — Trilha de Auditoria Diretrizes CFMV/POA 2023

// ── NÍVEIS ───────────────────────────────────────────────────────
export const NIVEIS = [
  { level: 1, nome: "RT Iniciante",    minScore:  0, maxScore: 39, emoji: "🥚", cor: "#9e9e9e",
    descricao: "Primeiros passos na conformidade — muito a construir." },
  { level: 2, nome: "RT em Formação",  minScore: 40, maxScore: 59, emoji: "🌱", cor: "#66bb6a",
    descricao: "Base sendo construída — parabéns pelo começo." },
  { level: 3, nome: "RT Conformante",  minScore: 60, maxScore: 74, emoji: "⚡", cor: "#42a5f5",
    descricao: "Conformidade básica atingida — continue avançando." },
  { level: 4, nome: "RT Experiente",   minScore: 75, maxScore: 84, emoji: "🏆", cor: "#ffa726",
    descricao: "Domínio sólido das exigências. Quase no topo." },
  { level: 5, nome: "RT Blindado",     minScore: 85, maxScore: 94, emoji: "🛡️", cor: "#ab47bc",
    descricao: "Conformidade robusta. Escudo regulatório ativo." },
  { level: 6, nome: "RT de Elite",     minScore: 95, maxScore:100, emoji: "⭐", cor: "#ef5350",
    descricao: "Compliance de excelência. Referência nacional." },
];
...
export const XP = {
  ...
  secao_100_porcento:       30,   // bônus por seção 100% verde
  primeiro_sem_critico:     75,   // primeira auditoria sem NC crítica
  streak_3_meses:          100,   // 3 auditorias seguidas com score ≥ 75%
};

// ── PENALIDADES DE SCORE (não afetam XP, apenas o score % da auditoria) ─────
export const PENALIDADES = {
  item_critico_nc:   -15,  // por item crítico NC
  item_maior_nc:      -5,  // por item maior NC
};
...
const BADGES_POA = [
  ...
  // ── REGISTRO E ROTULAGEM ───────────────────────────────────────
  {
    ...
    cor: "#e65100",
    criterio: (_, auditorias) =>
      auditorias.some(a => (a.score ?? 0) >= 80 &&
  ...
  },
  {
    ...
    criterio: (_, auditorias) =>
      auditorias.some(a =>
        a.itensConformes?.some(i => i.id === "POA_REG_2")),
  },
  {
  ...
  },
  {
    ...
    criterio: (_, auditorias) =>
      auditorias.some(a =>
        a.itensConformes?.some(i => i.id === "POA_DOC_5")),
  },
  // ── QUALIDADE / 13 PACs ────────────────────────────────────────
  {
    id: "13_pacs_ok",
  ...
  },
  {
    ...
    descricao: "Plano APPCC completo (11 etapas) elaborado e validado",
    categoria: "qualidade",
    xp: 250,
    icon: "AccountTree",
    cor: "#c62828",
    criterio: (_, auditorias) =>
      auditorias.some(a =>
        a.documentosGerados?.includes("POA_DOC_APPCC_01")),
  },
  {
    id: "recall_simulado",
    nome: "Plano de Escape",
    descricao: "Simulação de recall executada com 100% do lote rastreado em ≤4h",
    categoria: "qualidade",
    xp: 200,
    icon: "CrisisAlert",
    cor: "#c62828",
    criterio: (_, auditorias) =>
      auditorias.some(a =>
        a.itensConformes?.some(i => i.id === "POA_RCL_4")),
  },
  {
    id: "agua_conforme",
    nome: "Água Pura",
    ...
    criterio: (_, auditorias) => {
      const agua_ok = auditorias.filter(a =>
        a.itensConformes?.some(i => i.id === "POA_AGUA_3" || i.id === "POA_AGUA_4")
      );
  ...
  },
  {
    ...
    descricao: "Todos os instrumentos calibrados e com certificado vigente",
    categoria: "qualidade",
    xp: 150,
    icon: "Tune",
    cor: "#c62828",
  ...
  // ── AMBIENTAL ─────────────────────────────────────────────────
  {
    id: "guardiao_verde",
    nome: "Guardião Verde",
    descricao: "Licença Ambiental vigente + PGRS implementado",
    categoria: "ambiental",
    xp: 150,
    icon: "Eco",
    cor: "#2e7d32",
    criterio: (userData) =>
      userData?.vencLicAmbiental && new Date(userData.vencLicAmbiental) > new Date(),
  },
  {
    id: "residuos_zero",
    nome: "Resíduos Zero",
  ...
  // ── BEM-ESTAR ANIMAL ──────────────────────────────────────────
  {
    ...
    descricao: "BEA conforme por 1 mês de monitoramento diário",
    categoria: "bea",
    xp: 200,
    icon: "Pets",
    cor: "#6a1b9a",
  ...
  // ── ESPECIAIS / COMBOS ────────────────────────────────────────
  {
    ...
    descricao: "Score ≥ 85% em 6 auditorias consecutivas",
    categoria: "especial",
    xp: 500,
    icon: "Stars",
    cor: "#f57f17",
    criterio: (_, auditorias) => {
      const sorted = [...auditorias].sort(
  ...
  },
  {
    id: "rt_elite",
    nome: "RT de Elite",
    descricao: "Level 6 atingido — score ≥ 95% em auditoria",
    categoria: "especial",
    xp: 1000,
    icon: "WorkspacePremium",
    cor: "#ef5350",
    criterio: (_, auditorias) =>
      auditorias.some(a => (a.score ?? 0) >= 95),
  },
];
...
const MISSOES_POA = [
  {
    ...
    passos: [
      ...
      { id: "m1_p3", texto: "Verificação de impedimentos concluída", campo: "impedimentosVerificados" },
      { id: "m1_p4", texto: "Contrato de prestação de serviços vigente", campo: "contratoVigente" },
      { id: "m1_p5", texto: "Registro do estabelecimento SIF/SIE/SIM ativo", campo: "vencSif" },
    ],
    recompensaXP: 250,
  ...
  },
  {
    ...
    cor: "#e65100",
    passos: [
      { id: "m2_p1", texto: "Produtos registrados no PGA/SIGSIF", itemAuditoria: "POA_REG_1" },
      { id: "m2_p2", texto: "Rótulos aprovados e RTIQs observados", itemAuditoria: "POA_REG_2" },
      { id: "m2_p3", texto: "MDES validado e aprovado pelo órgão de inspeção", itemAuditoria: "POA_DOC_5" },
      { id: "m2_p4", texto: "Dados estatísticos enviados ao SIF no mês atual", itemAuditoria: "POA_SIF_1" },
    ],
    recompensaXP: 400,
  ...
  },
  {
    ...
    passos: [
      ...
      { id: "m3_p2",  texto: "PAC 02 — Saúde e higiene dos manipuladores", itemAuditoria: "POA_MAN_1" },
      ...
      { id: "m3_p5",  texto: "PAC 06 — Calibração de instrumentos", itemAuditoria: "POA_CAL_1" },
      { id: "m3_p6",  texto: "PAC 07 — PPHO verificado", itemAuditoria: "POA_CK_04" },
      { id: "m3_p7",  texto: "PAC 08 — Água de abastecimento com laudo", itemAuditoria: "POA_AGUA_3" },
      { id: "m3_p8",  texto: "PAC 09 — Controle de pragas in loco", itemAuditoria: "POA_CK_03" },
      { id: "m3_p9",  texto: "PAC 10 — Matéria-prima com laudo e fornecedor qualificado", itemAuditoria: "POA_MP_1" },
      { id: "m3_p10", texto: "PAC 11 — Temperaturas de câmaras registradas", itemAuditoria: "POA_CAL_1" },
      { id: "m3_p11", texto: "PAC 12 — BEA verificado (quando aplicável)", itemAuditoria: "POA_BEA_1" },
      { id: "m3_p12", texto: "PAC 13 — Rastreabilidade e recall com simulação", itemAuditoria: "POA_RCL_4" },
      { id: "m3_p13", texto: "Sistemas digitais de PAC com auditabilidade garantida", itemAuditoria: "POA_TI_2" },
    ],
    recompensaXP: 600,
  ...
  },
  {
    ...
    cor: "#6a1b9a",
    passos: [
      { id: "m4_p1", texto: "Equipe APPCC formada e registrada", itemAuditoria: "POA_FOR_1" },
      { id: "m4_p2", texto: "Descrição do produto e uso pretendido", itemAuditoria: "POA_REG_3" },
      { id: "m4_p3", texto: "Fluxograma de processo verificado in loco", itemAuditoria: "POA_DOC_5" },
      { id: "m4_p4", texto: "Análise de perigos concluída (B/Q/F)", itemAuditoria: "POA_FOR_1" },
      { id: "m4_p5", texto: "PCCs identificados com árvore decisória", itemAuditoria: "POA_FOR_1" },
      { id: "m4_p6", texto: "Limites críticos definidos e documentados", itemAuditoria: "POA_CAL_1" },
      { id: "m4_p7", texto: "Monitoramento dos PCCs implantado e registrado", itemAuditoria: "POA_CAL_1" },
      { id: "m4_p8", texto: "Ações corretivas definidas para cada PCC", itemAuditoria: "POA_FOR_1" },
      { id: "m4_p9", texto: "Verificação periódica do sistema APPCC", itemAuditoria: "POA_CAL_6" },
      { id: "m4_p10", texto: "Documentação arquivada por no mínimo 2 anos", itemAuditoria: "POA_TI_3" },
  ...
  },
  {
    ...
    cor: "#c62828",
    passos: [
      { id: "m5_p1", texto: "Plano de recolhimento (RDC 655/2022) elaborado", templateGerado: "POA_DOC_RECALL_RDC655_01" },
      { id: "m5_p2", texto: "Lote simulado definido e registrado", itemAuditoria: "POA_RCL_1" },
      { id: "m5_p3", texto: "Exercício executado — 100% do lote rastreado", itemAuditoria: "POA_RCL_4" },
      { id: "m5_p4", texto: "Relatório de simulação arquivado com resultado", itemAuditoria: "POA_RCL_6" },
      { id: "m5_p5", texto: "Próxima simulação programada", itemAuditoria: "POA_RCL_4" },
    ],
    recompensaXP: 400,
...
// Baseadas diretamente nas seções das Diretrizes CFMV/CRMVs 2023
const SECOES_TRILHA_POA = [
  ...
  },
  {
    ...
    itens: [
      { id:"T_C_01", class:"CRÍTICO", peso:10, desc:"Instalações em conformidade com o RIISPOA e Portaria MAPA 368/1997 (BPF)?", ref:"RIISPOA; Portaria MAPA 368/1997" },
  ...
  },
  {
    ...
    descricao: "Licença ambiental, PGRS, efluentes, crueldade animal",
    icon: "Eco",
    cor: "#1b5e20",
    corBg: "#f1f8e9",
    pontos: 10,
  ...
  },
  {
    ...
    itens: [
      { id:"T_G_01", class:"CRÍTICO", peso:10, desc:"BEA — Abate humanitário conforme Portaria MAPA 365/2021 e SDA 631/2022? Planilha preenchida?", ref:"Portaria MAPA 365/2021; SDA 631/2022" },
...
// ── BADGES (PEQUENOS ANIMAIS) ────────────────────────────────────────────────────────
const BADGES_PET = [
  { id: "art_vigente", nome: "ART em Dia", descricao: "ART averbada e vigente no CRMV", categoria: "base_legal", xp: 100, icon: "VerifiedUser", cor: "#1565c0", criterio: (userData) => userData?.vencCrmv && new Date(userData.vencCrmv) > new Date() },
  { id: "alvara_sanitario", nome: "Licença Dourada", descricao: "Alvará Sanitário e Licenças municipais em dia", categoria: "base_legal", xp: 100, icon: "DomainVerification", cor: "#1565c0", criterio: (userData) => userData?.vencAlvara && new Date(userData.vencAlvara) > new Date() },
  { id: "bisturi_de_ouro", nome: "Bisturi de Ouro", descricao: "Centro cirúrgico e fluxos de esterilização sem Não Conformidades", categoria: "qualidade", xp: 200, icon: "MedicalServices", cor: "#c62828", criterio: (_, auditorias) => auditorias.some(a => a.itensConformes?.some(i => i.id === "T_PET_C_01") && a.itensConformes?.some(i => i.id === "T_PET_C_02")) },
  { id: "prontuario_impecavel", nome: "Prontuário Impecável", descricao: "100% dos prontuários médicos contêm assinatura, TCLE e guarda digital", categoria: "qualidade", xp: 150, icon: "HistoryEdu", cor: "#c62828", criterio: (_, auditorias) => auditorias.some(a => a.itensConformes?.some(i => i.id === "T_PET_C_04")) },
  { id: "guardiao_receita", nome: "Guardião da Receita", descricao: "SIPEAGRO, livros de psicotrópicos e armários controlados auditados com sucesso", categoria: "normas", xp: 250, icon: "Lock", cor: "#e65100", criterio: (_, auditorias) => auditorias.some(a => a.itensConformes?.some(i => i.id === "T_PET_D_01")) },
  { id: "clinica_sustentavel", nome: "Clínica Sustentável", descricao: "PGRSS implementado e descarte correto de perfurocortantes / infectantes", categoria: "ambiental", xp: 150, icon: "Nature", cor: "#2e7d32", criterio: (_, auditorias) => auditorias.some(a => a.itensConformes?.some(i => i.id === "T_PET_E_01")) },
];

// ── MISSÕES (PEQUENOS ANIMAIS) ───────────────────────────────────────────────────────
const MISSOES_PET = [
  { id: "m_pet_fundacao", nome: "Fundação Segura", descricao: "Mantenha a base legal (ART, CRMV, Alvará) rigorosamente em dia", emoji: "⚖️", cor: "#1565c0", recompensaXP: 250, recompensaBadge: "art_vigente", sequencia: 1, passos: [{ id: "m_p1", texto: "ART averbada e vigente", campo: "vencCrmv" }, { id: "m_p2", texto: "Alvará Sanitário vigente", campo: "vencAlvara" }, { id: "m_p3", texto: "Registro no CRMV ativo", campo: "vencCrmv" }] },
  { id: "m_pet_farmacia", nome: "Farmácia Blindada", descricao: "Assegure o controle absoluto sobre psicotrópicos e validades", emoji: "💊", cor: "#e65100", recompensaXP: 300, recompensaBadge: "guardiao_receita", sequencia: 2, desbloqueiaApos: "m_pet_fundacao", passos: [{ id: "f_p1", texto: "Estoque no SIPEAGRO e Livro físico batem", itemAuditoria: "T_PET_D_01" }, { id: "f_p2", texto: "Armário de controlados com chave sob posse do RT", itemAuditoria: "T_PET_D_02" }] },
  { id: "m_pet_biosseguranca", nome: "Mestre da Biossegurança", descricao: "Implemente fluxos e barreiras contra infecções hospitalares", emoji: "🛡️", cor: "#c62828", recompensaXP: 350, recompensaBadge: "bisturi_de_ouro", sequencia: 3, desbloqueiaApos: "m_pet_farmacia", passos: [{ id: "b_p1", texto: "Barreiras físicas implementadas", itemAuditoria: "T_PET_B_01" }, { id: "b_p2", texto: "Centro cirúrgico independente", itemAuditoria: "T_PET_C_01" }] },
  { id: "m_pet_sustentavel", nome: "Sustentabilidade", descricao: "Faça a gestão correta de resíduos (PGRSS)", emoji: "🌱", cor: "#2e7d32", recompensaXP: 200, recompensaBadge: "clinica_sustentavel", sequencia: 4, desbloqueiaApos: "m_pet_biosseguranca", passos: [{ id: "s_p1", texto: "PGRSS implementado", itemAuditoria: "T_PET_E_01" }] },
];

// ── SEÇÕES DA TRILHA DE AUDITORIA (PEQUENOS ANIMAIS) ────────────────────────────────
const SECOES_TRILHA_PET = [
  { id: "A", letra: "A", nome: "Base Legal e Documental", descricao: "ART, CRMV, Alvarás, PCMSO", icon: "Gavel", cor: "#1565c0", corBg: "#e3f2fd", pontos: 15, referencia: "Resolução CFMV 1275/2019", itens: [
  ...
  { id: "B", letra: "B", nome: "Infraestrutura e Biossegurança", descricao: "Barreiras físicas, áreas limpas e higienização", icon: "Factory", cor: "#2e7d32", corBg: "#e8f5e9", pontos: 20, referencia: "Resolução CFMV 1275/2019", itens: [
  ...
  { id: "E", letra: "E", nome: "Gerenciamento de Resíduos (PGRSS)", descricao: "Lixo infectante, perfurocortantes e abrigo", icon: "Nature", cor: "#1b5e20", corBg: "#f1f8e9", pontos: 15, referencia: "RDC ANVISA 222/2018", itens: [
...
export function calcularScoreTrilha(respostas, secoes_trilha) {
  ...
  secoes_trilha.forEach(secao => {
    secao.itens.forEach(item => {
      ...
      if (resp === "na") return; // N/A não conta

      if (item.class === "CRÍTICO") {
        pontuacaoMaxima += item.peso;
        if (resp === "conforme") pontuacaoObtida += item.peso;
        else { pontuacaoObtida += PENALIDADES.item_critico_nc; criticos_nc++; }
      } else if (item.class === "MAIOR") {
        pontuacaoMaxima += item.peso;
        if (resp === "conforme") pontuacaoObtida += item.peso;
        else { pontuacaoObtida += PENALIDADES.item_maior_nc; maiores_nc++; }
      } else {
        pontuacaoMaxima += item.peso;
        if (resp === "conforme") pontuacaoObtida += item.peso;
      }
...
export function calcularXPAuditoria(resultado, auditorias_anteriores) {
  ...
  // Bônus primeira auditoria sem críticos
  const nunca_sem_critico = !auditorias_anteriores.some(a => (a.criticasNC ?? 99) === 0);
